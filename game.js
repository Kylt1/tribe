'use strict';
const SERVER_URL = null; // auto-fetched from /tunnel-url
...
function makeTreeSprite(){
  return getSprite('tree',60,64,(c,w,h)=>{
    c.fillStyle='#5a2e10'; c.fillRect(27,42,6,18);
    c.fillStyle='#1e5010'; c.beginPath(); c.arc(30,28,22,0,Math.PI*2); c.fill();
    c.fillStyle='#2d6e1e'; c.beginPath(); c.arc(29,20,17,0,Math.PI*2); c.fill();
    c.fillStyle='#3d8428'; c.beginPath(); c.arc(30,14,12,0,Math.PI*2); c.fill();
    c.fillStyle='rgba(255,255,200,.1)'; c.beginPath(); c.arc(25,11,5,0,Math.PI*2); c.fill();
  });
}
function makeBushSprite(){
  return getSprite('bush',44,36,(c,w,h)=>{
    c.fillStyle='#1e5810'; c.beginPath(); c.arc(22,20,16,0,Math.PI*2); c.fill();
    c.fillStyle='#2e7018'; c.beginPath(); c.arc(13,18,11,0,Math.PI*2); c.fill();
    c.fillStyle='#3a8820'; c.beginPath(); c.arc(30,17,10,0,Math.PI*2); c.fill();
    [[13,22],[22,18],[31,22],[18,26],[27,25]].forEach(([x,y])=>{
      c.fillStyle='#b02828'; c.beginPath(); c.arc(x,y,2,0,Math.PI*2); c.fill();
    });
  });
}
function makeStoneSprite(){
  return getSprite('stone',52,48,(c,w,h)=>{
    c.fillStyle='#5a5a68';
    c.beginPath(); c.moveTo(4,36); c.lineTo(10,8); c.lineTo(28,4); c.lineTo(46,12); c.lineTo(48,32); c.lineTo(34,42); c.closePath(); c.fill();
    c.fillStyle='#8888a0';
    c.beginPath(); c.moveTo(12,12); c.lineTo(28,6); c.lineTo(42,16); c.lineTo(30,22); c.closePath(); c.fill();
    c.fillStyle='rgba(255,255,255,.2)'; c.beginPath(); c.arc(18,14,2.5,0,Math.PI*2); c.fill();
  });
}
function makeCactusSprite(){
  return getSprite('cactus',44,58,(c,w,h)=>{
    c.fillStyle='#306620'; c.strokeStyle='#1e4414'; c.lineWidth=1.5;
    c.fillRect(17,6,10,48); c.strokeRect(17,6,10,48);
    c.fillRect(5,20,14,8); c.strokeRect(5,20,14,8);
    c.fillRect(25,14,14,8); c.strokeRect(25,14,14,8);
    c.strokeStyle='rgba(255,255,200,.5)'; c.lineWidth=1;
    [[18,16],[18,28],[25,22],[25,34],[18,40]].forEach(([x,y])=>{
      c.beginPath(); c.moveTo(x,y); c.lineTo(x<22?x-4:x+4,y); c.stroke();
    });
  });
}
...
function getStructSprite(type){
  ...
  if(type==='wall'){
    ...
    c.fillRect(cx-24,cy-24,48,48); c.strokeRect(cx-24,cy-24,48,48);
...
function drawPlayer(p){
  ...
  const bw=R*2.8|0;
  ctx.fillStyle='rgba(0,0,0,.45)'; ctx.fillRect(-bw>>1,-R-13,bw,7);
  ctx.fillStyle=hpPct>.5?'#30c050':hpPct>.25?'#e0a020':'#e03020';
  ctx.fillRect(-bw>>1,-R-13,(bw*hpPct)|0,7);
...
document.getElementById('btn-play').addEventListener('click',async()=>{
  ...
  // Try to auto-fetch tunnel URL from cloudflare metrics via server endpoint
  const KNOWN='wss://funeral-the-named-pointer.trycloudflare.com';
  let url=KNOWN;
  try{
    const r=await fetch(KNOWN.replace('wss://','https://')+'/tunnel-url');
